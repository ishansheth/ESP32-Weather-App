...
#define EXAMPLE_PIN_NUM_SCLK           23
...
//uint8_t uart_rx_data_buffer[25];
...
extern SemaphoreHandle_t wifiReadySemaphore;
...
//	init_get_spiffs_info();
	...
	ESP_LOGI(TAG, "Create get weather data task");	
	xTaskCreate(get_weather_data, "get_weather_data", EXAMPLE_LVGL_TASK_STACK_SIZE, NULL, 2, NULL);
			
    ESP_LOGI(TAG, "Create LVGL task");
    xTaskCreate(example_lvgl_port_task, "LVGL", EXAMPLE_LVGL_TASK_STACK_SIZE*2, NULL, 3, NULL);
...
//{
//	gpio_pullup_en(TXD_PIN);
//	gpio_pullup_en(RXD_PIN);
//	
...
//		if(start_receive)
//		{
//			const int rxbytes = uart_read_bytes(UART_NUM,uart_rx_data_buffer,25,1000/portTICK_PERIOD_MS);
//			if(rxbytes > 0)
//			{
...
//				vTaskDelay(pdMS_TO_TICKS(100));
//				ESP_LOGI(RX_TAG,"Read from UART: %s: %d",uart_rx_data_buffer,rxbytes);
//				_lock_acquire(&lvgl_api_lock);		
//				set_received_uart_text(uart_rx_data_buffer);
//				_lock_release(&lvgl_api_lock);
//			}		 			
//		}
//		
